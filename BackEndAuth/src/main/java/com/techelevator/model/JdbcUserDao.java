...
public class JdbcUserDao implements UserDao {
     ...
     */
    @Override
    public User saveUser(String userName, String password, String role, String image) {
        byte[] salt = passwordHasher.generateRandomSalt();
        String hashedPassword = passwordHasher.computeHash(password, salt);
        String saltString = new String(Base64.encode(salt));
        long newId = jdbcTemplate.queryForObject("INSERT INTO users(username, password, salt, role, image) VALUES (?, ?, ?, ?, ?) RETURNING id", Long.class, userName,
                hashedPassword, saltString, role, image);
    ...
    public static User mapResultToUser(SqlRowSet results) {
        User user = new User();
    ...
    public User getUserByUsername(String username) {
        String sqlSelectUserByUsername = "SELECT id, username, role, image FROM users WHERE username = ?";
